...
const presentation = computed(() => presentEvaluation(props.evaluation.status, props.evaluation.verdict));
</script>
...
<template>
  <section class="evaluation-panel" :data-status="evaluation.status">
    ...
    <template v-else>
      <p class="eyebrow">평가 결과</p><h2>{{ presentation.label }}<span v-if="evaluation.score !== null"> · {{ evaluation.score }}점</span></h2>
