...
} from "@/composables/useAdminQuestionEditor";
import {
  ADMIN_PAGE_SIZE,
  fetchAllPages,
  normalizedPage,
...
async function load() {
  ...
  try {
    ...
    const validPage = normalizedPage(page.value, questionPage.totalPages);
...
async function loadRelations() {
  ...
  try {
    const [allTopics, allConcepts] = await Promise.all([
      ...
      fetchAllPages((candidatePage, size) => fetchConcepts({
        active: true,
        page: candidatePage,
        size
      })),
    ...
    <div v-else class="admin-editor-layout">
      ...
      <section class="admin-panel">
        ...
        <form class="admin-form" @submit.prevent="submit">
          ...
          <label>난이도<select v-model="form.difficulty">
              <option>BASIC</option>
          ...
          <div class="section-heading">
            ...
            <div v-if="selected.status === 'DRAFT'" class="criteria-add"><select
                ...
                <option v-for="concept in availableConcepts" :key="concept.id" :value="concept.id">
                  {{ concept.name }}</option>
              </select><button :disabled="newCriterionConceptId === undefined"
                @click="addCriterion">Concept 추가</button></div>
          </div>
