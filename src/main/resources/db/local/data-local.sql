INSERT INTO topic (parent_id, code, name)
SELECT NULL, 'OPERATING_SYSTEM', '운영체제'
WHERE NOT EXISTS (SELECT 1 FROM topic WHERE code = 'OPERATING_SYSTEM');

INSERT INTO concept (topic_id, code, name, description)
SELECT t.id, 'PROCESS_THREAD', '프로세스와 스레드', '실행 단위와 자원 공유 방식의 차이'
FROM topic t
