...
class EvaluationWorkerTest {
    ...
    void tearDown() {
        try {
            ...
            port.calledOutsideTransaction = false;
        }
    }
    ...
    private void awaitEvaluation(Long answerId) throws InterruptedException {
        ...
        while (System.nanoTime() < deadline) {
            if (evaluationRepository.findByAnswerId(answerId).orElseThrow().getStatus() == EvaluationStatus.EVALUATED) {
                return;
            }
            Thread.sleep(10);
        }
        assertThat(evaluationRepository.findByAnswerId(answerId).orElseThrow().getStatus())
